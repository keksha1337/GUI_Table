...
public:
	...
	std::shared_ptr<Table> CreateTable(GUILayer& fabric, float x, float y, float width, float height, TextStyle *tst, GUIStyle *gst);
	std::shared_ptr<Table> CreateTable(Vector2f xy, float width, float height);
	std::shared_ptr<Table> CreateTable(float x, float y, Vector2f size);
	std::shared_ptr<Table> CreateTable(Vector2f xy, Vector2f size);
