...
std::shared_ptr<Table> GUILayer::CreateTable(GUILayer& fabric, float x, float y, float width, float height, TextStyle *tst, GUIStyle *gst)
{
	std::shared_ptr<Table> table(new Table(&fabric, *window, x, y, width, height, tst, gst));
	elements.push_back(table);
	return table;
}

std::shared_ptr<Table> GUILayer::CreateTable(Vector2f xy, float width, float height)
{
	std::shared_ptr<Table> table(new Table(*window, xy, width, height));
	elements.push_back(table);
	return table;
}

std::shared_ptr<Table> GUILayer::CreateTable(float x, float y, Vector2f size)
{
	std::shared_ptr<Table> table(new Table(*window, x, y, size));
	elements.push_back(table);
	return table;
}

std::shared_ptr<Table> GUILayer::CreateTable(Vector2f xy, Vector2f size)
{
	std::shared_ptr<Table> table(new Table(*window, xy, size));
	elements.push_back(table);
	return table;
}